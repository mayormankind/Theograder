...
import { ArrowLeft, Loader2, CheckCircle2, X, RefreshCw, BookOpen, Calendar, Users, FileText } from 'lucide-react';
import { cn } from '@/lib/utils';
import { toast } from 'sonner';

type GradingStatus = 'queued' | 'ocr' | 'segmenting' | 'grading' | 'done' | 'error';
...
export default function ExamDetailPage() {
  ...
  const updateScriptEntry = (scriptId: string, updates: Partial<ScriptGradingEntry>) => {
    setScripts(prev => prev.map(s => 
  ...
  const processingCount = scripts.filter(s => ['ocr', 'segmenting', 'grading'].includes(s.status)).length;
  const errorCount = scripts.filter(s => s.status === 'error').length;

  const progress = uploadedCount > 0 ? Math.round((gradedCount / uploadedCount) * 100) : 0;
    ...
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-slate-500 mt-0.5">{exam.courseCode} · {exam.courseName}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <p className="text-lg font-semibold text-slate-800">{exam.totalMarks}</p>
        </div>
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            <Calendar size={16} className="text-slate-400" />
            <p className="text-xs text-slate-400 font-medium uppercase tracking-wide">Exam Date</p>
          </div>
          <p className="text-lg font-semibold text-slate-800">{exam.examDate ? new Date(exam.examDate).toLocaleDateString() : 'Not set'}</p>
        </div>
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            <Users size={16} className="text-slate-400" />
            <p className="text-xs text-slate-400 font-medium uppercase tracking-wide">Scripts</p>
          </div>
          <p className="text-lg font-semibold text-slate-800">{uploadedCount}</p>
        </div>
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            <FileText size={16} className="text-slate-400" />
            <p className="text-xs text-slate-400 font-medium uppercase tracking-wide">Rubric</p>
          </div>
          <p className="text-lg font-semibold text-slate-800">{hasRubric ? 'Created' : 'Not created'}</p>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="flex flex-col gap-4 p-5 border-b border-slate-100">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-base font-semibold text-slate-800">Script Grading</h2>
              <p className="text-sm text-slate-500 mt-0.5">
                {uploadedCount} scripts uploaded · {gradedCount} graded · {pendingCount} pending
              </p>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                title={!hasRubric ? 'Create a rubric before grading' : uploadedCount === 0 ? 'Upload scripts first' : ''}
              >
          ...
          {isGrading && (
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <p className="text-xs text-slate-500">{gradedCount} of {uploadedCount} scripts processed</p>
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                ...
                scripts.map((script) => (
                  <tr key={script.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    <td className="px-5 py-4">
                      <div>
                        <p className="text-sm font-medium text-slate-800">{script.studentName}</p>
                        <p className="text-xs text-slate-500">{script.studentId}</p>
                      </div>
                    </td>
                    <td className="px-5 py-4">
                      <p className="text-sm text-slate-600 truncate max-w-[200px]">{script.fileName}</p>
                    </td>
                    <td className="px-5 py-4">
                      ...
                      {script.status === 'segmenting' && (
                        <div className="flex items-center gap-2">
                          <Loader2 size={14} className="animate-spin text-blue-500" />
                          <span className="text-sm text-blue-600">Reading answers...</span>
                        </div>
                      )}
                      {script.status === 'grading' && (
                        <div className="flex items-center gap-2">
                          <Loader2 size={14} className="animate-spin text-blue-500" />
                          <span className="text-sm text-blue-600">Grading...</span>
                        </div>
                      )}
                      {script.status === 'done' && (
                        <div className="flex items-center gap-2">
                      ...
                      {script.status === 'error' && (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-red-600 truncate max-w-[200px]" title={script.error || 'Error'}>
                            {script.error || 'Error'}
                          </span>
                        </div>
                      )}
                    ...
                    <td className="px-5 py-4">
                      {script.status === 'done' && script.score !== null ? (
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-semibold text-slate-800">{script.score}</span>
                            <span className="text-sm text-slate-500">/ {script.totalMarks}</span>
                          </div>
                          {script.totalMarks && script.totalMarks > 0 && (
                            <div className="h-1.5 w-24 rounded-full bg-slate-100 mt-1">
                              <div
                                className="h-1.5 rounded-full bg-teal-500"
                                style={{ width: `${(script.score / script.totalMarks) * 100}%` }}
                              />
                            </div>
                          )}
                        </div>
