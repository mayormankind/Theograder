    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
        ...
        <p className="text-xs text-slate-400">
          Select a script from the Scripts page to view its report
        </p>
        <button
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        {/* Header Banner */}
        <div className="border-b border-slate-100 bg-[#0f1f3d] px-5 sm:px-8 py-6">
          <div className="flex flex-col sm:flex-row items-center sm:items-start justify-between gap-6 text-center sm:text-left">
            <div className="w-full sm:w-auto">
              <p className="text-[10px] font-bold uppercase tracking-widest text-white/40 mb-1">
                TheoGrader — Grading Report
              </p>
              <h2 className="text-lg sm:text-xl font-bold text-white mb-0.5">
                {gradingResult.exam?.title || "Examination"}
              </h2>
              <p className="text-xs sm:text-sm text-white/60">
                {gradingResult.exam?.courseCode} ·{" "}
                {gradingResult.exam?.courseName}
              </p>
            </div>
            <div
              className={cn(
                "flex flex-col items-center justify-center rounded-xl px-6 py-4 ring-1",
                bg,
                ring,
              )}
            >
              <p className={cn("text-4xl sm:text-4xl font-black", color)}>
                {grade}
              </p>
              <p
                className={cn(
        ...
        <div className="grid grid-cols-1 xs:grid-cols-2 gap-4 sm:grid-cols-4 border-b border-slate-100 px-5 sm:px-8 py-5">
          {[
            ...
            {
              label: "Date Graded",
              value: gradingResult.gradedAt
                ? new Date(gradingResult.gradedAt).toLocaleDateString()
                : "N/A",
            },
            { label: "Graded By", value: "TheoGrader" },
          ].map((item) => (
            <div key={item.label} className="text-center xs:text-left">
        ...
        {/* Score Summary */}
        <div className="grid grid-cols-1 xs:grid-cols-2 gap-3 sm:grid-cols-4 border-b border-slate-100 px-5 sm:px-8 py-5 bg-slate-50/50">
          {[
            {
              label: "Total Score",
            ...
            {
              label: "Questions",
              value: `${gradingResult.questions?.length || 0}`,
              icon: FileText,
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100">
                  ...
                  <th className="pb-2 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wide hidden sm:table-cell">
                    Similarity
                  </th>
                  <th className="pb-2 text-left text-[11px] font-semibold text-slate-400 uppercase tracking-wide hidden sm:table-cell">
                    Confidence
                        ...
                        {hasMissing ? (
                          <div className="flex items-center gap-1 text-amber-600">
                            ...
                            <span className="text-[10px] font-medium">
                              {q.missingConcepts.length} missing
                            </span>
                          </div>
                        ) : (
              ...
              <tfoot>
                <tr className="border-t-2 border-slate-200">
                  <td className="pt-3 text-xs font-bold text-slate-700 uppercase">
                    Total
                  </td>
                  <td className="pt-3 text-sm font-bold text-slate-900">
                    {totalScore}/{totalMax}
                  </td>
                  <td className="pt-3 hidden sm:table-cell text-sm text-slate-600">
                    —
                  </td>
                  <td className="pt-3 hidden sm:table-cell text-sm font-bold text-slate-700">
                    {avgConf}%
                  </td>
                  <td className="pt-3" />
                </tr>
        ...
        <div className="border-t border-slate-100 bg-slate-50/50 px-8 py-4">
          <p className="text-[10px] sm:text-[11px] text-slate-400">
            This report was generated by TheoGrader using OCR text extraction
            and Sentence-BERT semantic similarity analysis. Confidence scores
            below 70% are flagged for mandatory manual review. All overrides are
            logged.
          </p>
        </div>
      </div>
